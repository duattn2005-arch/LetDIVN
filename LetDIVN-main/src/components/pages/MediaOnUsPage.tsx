          ...
          {entries.map((entry) => {
            return (
              <div key={entry.id} className="relative group flex flex-wrap items-center gap-x-12 gap-y-5">
                <div className="w-[28rem] aspect-3/2 bg-slate-900 shrink-0">
                  <img src={entry.image} alt={entry.title} className="w-full h-full object-cover" />
                </div>

                <div className="w-20 text-center shrink-0">
                  <div className="text-5xl sm:text-6xl font-black" style={{ color: BRAND_PINK }}>
                    {entry.articleCount}
                  </div>
                ...
                <div className="w-20 text-center shrink-0">
                  <div className="text-5xl sm:text-6xl font-black text-orange-500">{entry.segmentCount}</div>
