...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const googleRedirectBtnRef = useRef<HTMLDivElement>(null);
  ...
  // In-app browsers (Messenger/Zalo/...) break the popup-based Google flow —
  // there's no window.opener left for it to report back to. Google's own
  // "Sign In With Google" button in redirect mode sidesteps that entirely:
  // the whole page navigates to Google and back, no popup involved.
  useEffect(() => {
    if (!isOpen || !inAppBrowser || view !== 'main') return;
    ...
    const renderGoogleButton = () => {
      ...
      google.accounts.id.initialize({
        client_id: clientId,
        ux_mode: 'redirect',
        login_uri: `${window.location.origin}/api/auth/google-onetap`,
      });
      if (googleRedirectBtnRef.current) {
        googleRedirectBtnRef.current.innerHTML = '';
        google.accounts.id.renderButton(googleRedirectBtnRef.current, {
          type: 'standard',
          theme: 'outline',
          size: 'large',
          text: 'signin_with',
          ...
          <div className="space-y-2.5 mb-5">
            {inAppBrowser ? (
              <div className="space-y-1.5">
                <div ref={googleRedirectBtnRef} className="flex justify-center min-h-[44px]" />
                <p className="text-[10px] text-slate-500 text-center px-0.5">
                  Trang đang mở trong Messenger/Zalo nên dùng nút Google riêng ở trên (không mở popup, không bị treo màn hình).
                </p>
              </div>
            ) : (
              <button
