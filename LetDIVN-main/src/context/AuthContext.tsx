...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginWithGoogle = async (): Promise<UserProfile> => {
    ...
    if (!GOOGLE_CLIENT_ID) {
      throw new Error('Google sign-in is not configured. Please add VITE_GOOGLE_CLIENT_ID to the .env file (see the instructions in .env.example).');
    }
    if (typeof window === 'undefined' || !(window as any).google?.accounts?.oauth2) {
      throw new Error('Could not load the Google sign-in service. Please check your network connection and disable any ad blockers, then try again.');
    }
      ...
      const tokenClient = (window as any).google.accounts.oauth2.initTokenClient({
        ...
        error_callback: (err: any) => {
          if (err?.type === 'popup_closed') {
            reject(new Error('You closed the Google sign-in window before completing it.'));
          } else {
            reject(new Error(`Google sign-in failed: ${err?.type || 'unknown error'}. Check your Client ID and the authorized domains in the Google Cloud Console.`));
          }
        },
      });
  ...
  const loginWithFacebook = async (): Promise<UserProfile> => {
    if (typeof window === 'undefined' || !(window as any).FB) {
      throw new Error('Could not load the Facebook sign-in service. Please check your network connection and disable any ad blockers, then try again.');
    }
    ...
    const accessToken = await new Promise<string>((resolve, reject) => {
      (window as any).FB.login((response: any) => {
        if (response.authResponse?.accessToken) {
          ...
          reject(new Error('Facebook sign-in was cancelled or the authentication window was closed.'));
        }
  ...
  const loginWithEmail = async (emailOrUsername: string, pass: string): Promise<UserProfile> => {
    ...
    if (!trimmedInput || !pass) {
      throw new Error('Please enter both your account and password.');
    }
  ...
  const requestPasswordReset = async (email: string): Promise<void> => {
    ...
    if (!result.success) {
      throw new Error(result.message || 'Could not send the email. Please try again.');
    }
  };
