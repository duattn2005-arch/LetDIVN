...
interface WhoWeAreSectionEditorModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const WhoWeAreSectionEditorModal: React.FC<WhoWeAreSectionEditorModalProps> = ({
  isOpen,
  onClose,
  itemToEdit,
  onSave,
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-lg sm:text-xl font-black text-slate-900 flex items-center gap-2">
                  ...
                  <span>{itemToEdit ? 'Chỉnh Sửa Phần Nội Dung (Admin)' : 'Thêm Phần Nội Dung Mới (Admin)'}</span>
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  Nhập ảnh, tiêu đề và nội dung để thêm một phần mới trên trang Chúng Tôi Là Ai
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700">Tiêu đề *</label>
                <input
                  type="text"
                  required
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="VD: Our Mission, Our Journey..."
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700">Nội dung mô tả</label>
                <textarea
                  rows={4}
                  value={desc}
                  onChange={(e) => setDesc(e.target.value)}
                  placeholder="Nội dung chi tiết cho phần này..."
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                >
                  ...
                  <span>{itemToEdit ? 'Lưu Thay Đổi' : 'Thêm Phần Này'}</span>
                </button>
              </div>
            </form>
          </div>
