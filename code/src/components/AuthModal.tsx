...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleConfirmReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResetSuccessMsg('Password reset successful! Please log in with your new password.');
      setResetCode('');
      setNewPassword('');
      setTimeout(() => {
        setView('main');
      ...
      >
        ...
        <div className="bg-gradient-to-r from-slate-900 via-[#E81A7F] to-[#E81A7F] p-6 text-white text-center relative">
          ...
          <h3 className="text-2xl font-extrabold tracking-tight">
            ...
            {view === 'main' && (activeTab === 'login' ? 'Login' : 'Create New Account')}
          </h3>
          <p className="text-xs text-white/90 mt-1">
            {view === 'forgot-request' && 'Enter your registered email to receive a verification code'}
            {view === 'forgot-reset' && `Enter the 6-digit code sent to ${resetEmail}`}
            {view === 'main' && (activeTab === 'login'
              ? 'Welcome back to the green living movement'
              : 'Register an account to start joining campaigns')}
          </p>
        </div>

        {/* Tab switcher: Login / Register (hidden during forgot-password flow) */}
        {view === 'main' && (
        ...
        <div className="p-6">
          ...
          {view === 'main' && activeTab === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-3.5">
              ...
              >
                <span>{loading ? 'Checking account...' : 'Login Now'}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
          ...
          {view === 'main' && activeTab === 'register' && (
            <form onSubmit={handleRegisterSubmit} className="space-y-3">
              ...
              <button
                ...
                className="w-full bg-[#E81A7F] hover:bg-[#D01370] text-white font-bold text-sm py-3 rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer flex items-center justify-center gap-2 mt-2"
              >
                <span>{loading ? 'Saving account to database...' : 'Complete Registration'}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
