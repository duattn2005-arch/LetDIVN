      ...
      >
            ...
            <div className="text-center sm:text-left">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                ...
                <span className={`text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full ${
                  ...
                  {user.role === 'admin' ? '🛡️ Admin Account' : '🌱 Volunteer Account'}
                </span>
              </div>
              <p className="text-xs text-white/80 mt-1">
                Signed in via: <strong>{user.provider.toUpperCase()}</strong> • Member since: {user.joinedAt}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {isEditingAvatar && (
            <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold">Update your profile picture</span>
                <button
              ...
              <ImageUploadWidget
                ...
                aspectRatioLabel="1:1 square or 4:3 ratio"
              />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            ...
            <div className="bg-slate-50 dark:bg-slate-800/80 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700">
              <span className="text-[11px] font-bold text-slate-500 uppercase">Badge Tier</span>
          ...
          <div className="p-4 bg-slate-100 dark:bg-slate-800/70 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              {isAllowedAdminEmail(user.email) && (
                ...
                >
                  Switch to: {user.role === 'admin' ? 'Volunteer Account' : 'Admin Account'}
                </button>
              )}
